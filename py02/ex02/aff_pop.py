...
def main():
    """Plot the population expectancy
per years for France since 1800 to 2100"""
    pop = load("../datas/population_total.csv")
    if (pop is None):
        exit(1)
    pop.set_index("country", inplace=True)
    pop.columns = [int(year) for year in pop.columns]
    pop = pop.map(lambda x: int(float(x[:-1]) * 1000000)
                  if x.endswith("M") else int(float(x[:-1]) * 1000))
    plt.figure(figsize=(8, 6))
    plt.plot(pop.columns[:-50],
             pop.loc["France"][:-50],
             label="France",
             color="green")
    plt.plot(pop.columns[:-50],
